...
Page({
  ...
  formSubmit: function(e) {
    ...
    var specification = e.detail.value.specification //规格
    ...
    if (that.data.jinchang_num == false) {
      ...
      var weight = 0
      console.log(price)
      var danwei = that.data.danwei[that.data.index5]
  ...
  hidden2:function(){
    wx.request({
      url: getApp().globalData.url + '/api/specification' + '?goods_id=' + this.data.goodsId,
      method: 'GET',
      header: {
        'Content-Type': 'application/json',
        'X-Requested-With': 'XMLHttpRequest',
